...
for epoch in range(args.epoch_start, args.epoch_num):
    for i, batch in enumerate(train_dataloader):
        ...
        sameB=generatorA2B(real_B)
        loss_identity_B=criterion_pixelwise(sameB,real_B)
        sameA=generatorB2A(real_A)
        loss_identity_A=criterion_pixelwise(sameA,real_A)
        
        fakeB=generatorA2B(real_A)
        pred_fake=discriminatorB(fakeB,real_A)
        #计算GAN损失函数
        loss_GAN_A2B=criterion_GAN(pred_fake,valid)
        #计算得到像素级别的损失函数pixel_GAN
        pixel_GAN_A2B = criterion_pixelwise(fakeB, real_B)*100.0
        fakeA=generatorB2A(real_B)
        #计算虚假图片的得分
        pred_fake = discriminatorB(fakeA,real_B)
        loss_GAN_B2A = criterion_GAN(pred_fake,valid)
        pixel_GAN_B2A=criterion_pixelwise(fakeA,real_A)*100.0
        recovered_A = generatorB2A(fakeB)
        #计算cycleGAN的损失使得生成器的输入和输出具有相似的特征
        loss_cycle_ABA = criterion_pixelwise(recovered_A, real_A) * 10.0
        recovered_B = generatorA2B(fakeA)
        loss_cycle_BAB = criterion_pixelwise(recovered_B, real_B) * 10.0
        ...
        pred_fake = discriminatorA(fakeA.detach(),real_B)
        ...
        pred_fake = discriminatorB(fakeB.detach(),real_A)
        loss_D_fake = criterion_GAN(pred_fake, fake)
